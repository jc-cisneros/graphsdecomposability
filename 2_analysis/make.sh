...
mkdir -p "${MAKE_SCRIPT_DIR}/output/tables"
mkdir -p "${MAKE_SCRIPT_DIR}/output/figures"
mkdir -p "${MAKE_SCRIPT_DIR}/output/scalars"
...
# Simulation grid (Section 6 of the paper) and lambda sensitivity (appendix).
run_R run_simulations_extended.r     "${LOGFILE}" || exit 1
run_R run_lambda_sensitivity.r       "${LOGFILE}" || exit 1

# Empirical application: DDLY bank-volatility network (Section 7).
run_R run_ddly_application.r         "${LOGFILE}" || exit 1
) || false
